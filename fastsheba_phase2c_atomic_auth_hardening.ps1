...
if (
    $authController.Contains('DB::transaction(function () use ($validated, $mobile, $request): User')
    -and $otpService.Contains('public function consumeOtp(string $mobile): void')
    -and $socialService.Contains('DB::transaction(function () use (')
) {
    Write-Host ""
